...
> **執行角色：企劃**——關注指標變化能否導出行動;資料載入與品質檢查切**數值 / QA** 視角。統計方法依 CLAUDE.md「資料分析」段。
...
1. **總覽**:本期各指標數值,表格
2. **趨勢**:近 {{trend_window}} 走勢,plotnine 折線,`plt.rcParams["font.family"] = "Microsoft JhengHei"`
...
統計檢定(若有分群比較或 A/B):先查常態性 / 同質變異 / 獨立性,結果給 effect size + p-value。
