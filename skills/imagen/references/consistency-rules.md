...
2. **All characters: refined attractive features** (mobile gacha aesthetic — beautiful face, well-proportioned body). **Avoid** western cartoon / Disney / Pixar / caricature drift. Certain prompt words (e.g. "manic", "wild flying outward", "battle-tested") bias the model toward western-cartoon style; when such words are needed, explicitly exclude those styles in the EXCLUSION section.
...
> 繁中摘要：生圖前先掃專案同類資產；找到就鎖死其規格當預設、代表作當 `--reference`、Strict Rules 明寫「visually rhyme」、再跟使用者確認 lock。
...
2. Pass 1–2 representative existing assets as `--reference` **in addition to** the identity / subject ref — a concrete style anchor, not just descriptive words.
...
**Why**: existing project assets define the visual lock for new ones; re-inventing per request is the failure mode. One drifted asset in a roster / HUD / scene breaks the visual rhyme — the common case is a single asset generated without a project `--reference` that ends up with different proportions or line weight, only spotted after assembling everything side by side.
...
> 繁中摘要：AI 產出當參考圖會複利放大高頻雜訊（白點／一條條髮絲／碎花越畫越多）——身分走圖、風格走字、圖只給 gen-0 且適度縮圖。
...
One-line summary: **identity from the image, style from the text; the image is always gen-0 and moderately downscaled.** Applies to every `--reference` passed to the draw engine (draw-engines.md). References that are *not* AI-generated (photos, hand-drawn art) need no downscale, but the role-split wording in (3) is still good practice.
