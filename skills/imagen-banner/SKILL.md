...
description: |
  ...
  觸發：「廣宣圖」「宣傳圖」「banner」「活動圖」「促銷圖」「機台宣傳」「節慶宣傳」「驗收 banner」「這張 banner 幫我看」「照這份企劃做圖」。
---
...
### Phase 0｜有企劃就先吃企劃（不要先問問題）

企劃通常已經寫好**文案**與**想要的元素**，這兩樣正好是 prompt 最關鍵的兩段。有企劃就先讀完再說，不要一上來丟問題清單。

**來源三種**：

| 來源 | 怎麼讀 |
|---|---|
| Notion 連結 | 用環境裡的 Notion MCP（`fetch` 類工具）讀。**讀不到就停**——常見是該頁沒分享給 integration，或不在 MCP 認證的同一個 workspace。回報實際錯誤（404 / 權限），請對方把頁面分享給 integration、或直接貼內容、或匯出 Markdown。**絕不從標題猜內容** |
| 本機檔（.md／匯出檔／截圖） | `Read`；截圖直接看圖抄文案 |
| 直接貼上的文字 | 照用 |

**抽取對照表**（企劃欄位 → prompt 段落）：

| 企劃裡通常有 | 對應到 | 處理原則 |
|---|---|---|
| 活動名稱／主標 | `[TYPOGRAPHY]` Level 1 | **逐字照抄**，不潤飾 |
| 賣點句／副標 | Level 2 | 過長先問能不能縮，不自己改 |
| 按鈕文字 | Level 3 | 沒寫就沿用產品既有 CTA 慣例，並回報用了什麼 |
| 優惠數字／倍率／獎勵內容 | Level 2；節慶類直接是**王者** | 數字逐位核對 |
| **「想要的元素」清單**（角色／吉祥物／道具／節慶符號／場景） | `[HERO]`／`[MASCOT]`／`[WEALTH]`／`[BACKGROUND]` | **列出的元素全部要出現，不得自行刪減** |
| 檔期／節慶 | `[BACKGROUND]` 的節慶識別碼 | |
| 版位／尺寸 | `[SPEC]` ＋ UI 疊圖區 | 企劃最常漏這項，漏了就問 |
| 參考圖／競品連結／既有素材 | `--reference` | 見 common.md §5、§2 |

**三條紀律**：

1. **文案逐字照抄**。企劃有錯字也照抄，但回報一句請對方確認——不要默默改掉
2. **元素不得自行刪減**。企劃列了 5 個元素就要 5 個都在；真的塞不下（違反 A1 單一王者）→ **先講並取得同意**，不要默默拿掉
3. **企劃沒寫的才問**，一次問完（見 Phase 1）。企劃寫了的就不要再問一遍

企劃與鐵則衝突 → 一句話點出，**照企劃做**。

### Phase 1｜補問企劃沒寫的（缺一項就問，不要猜）

用 AskUserQuestion 一次問完**企劃裡找不到的項目**：
