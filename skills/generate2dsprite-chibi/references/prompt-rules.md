...
**Default for this skill: `cel_shaded_chibi`.** Always include the cel-shaded chibi style block (see SKILL.md "Write the prompt manually") unless the user explicitly overrides.

Available art styles:

- `cel_shaded_chibi` (DEFAULT): anime cel-shaded chibi sprite, 2.5-head proportions, clean bold black outlines, flat fills, two-tone shading (base + one shadow), no gradients, no painterly brushwork. Reference look: Eversoul / Idle Heroes / Disgaea / AFK Arena.
- `clean_hd`: clean hand-painted HD 2D game asset style, crisp silhouettes, smooth surfaces, low texture noise, controlled lighting, no chunky pixels. Use for map props matching HD map.
- `pixel_art` / `retro_pixel`: classic / 16-bit pixel art. **If the user wants pixel art, redirect them to the sister skill `generate2dsprite` instead — that one defaults to pixel art.**
...
Do not write `16-bit`, `retro JRPG`, or `chunky pixel-art` in this skill — those belong in `generate2dsprite`. For clean HD map props, explicitly say `Do not make pixel art`.
